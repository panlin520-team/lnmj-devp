...
public class EnumUtil {
  /*  //dataInput转换到Map
    ...
    public static <T extends BaseEnum> Map<String, String> getEnumToMap2(Class<T> baseEnum ){
        Map<String,String> map = new HashMap<>();
        for(T each:baseEnum.getEnumConstants()){
            map.put(each.getCode().toString(),each.getDesc());
        }
        return map;
    }
    ...
    //获取枚举类中的所有数据（code,desc）判断是否存在
    public static <T extends BaseEnum> Boolean judge(Class<T> baseEnum,Object param){
        Map<Integer,String> map = new HashMap<>();
        for(T each:baseEnum.getEnumConstants()){
            map.put(each.getCode(),each.getDesc());
        }
